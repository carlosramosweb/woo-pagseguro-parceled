...
if ( ! class_exists( 'WC_PagSeguro_Parceled' ) ) {
	...
	class WC_PagSeguro_Parceled {		
		 ...
		 */
		public static function code_css_archive_product() { 
		 ...
		 */
		public static function code_css_single_product() { 
		 ...
		 */
		public static function code_css_page_cart() { 
		 ...
		 */
		public static function includes_once() {
		 ...
		 */
		public function wc_product_parceled_loop() {
			...
			if( isset( $enabled ) && $enabled == "yes" && $installment_loop_product == "yes" ) {
				...
				<div class="p-woo-pagseguro-price">
					...
					<strong><?php _e('Sem Juros - PagSeguro', 'wc-pagseguro-parceled' ); ?></strong>
					</div>
				</div>
				<?php
				}
		...
		public function price_parceled( $price, $installments ) {
			...
			$total = $this->round_up( $cf * $valor_total, 2 );			
			return $total;
		}
		 ...
		 */
		public function wc_product_parceled_single_product() {
			global $post, $product;			
			$product = wc_get_product();
			$args = array();
			$pagseguro_settings = get_option( 'woo_pagseguro_parceled_settings' );
			...
			if( isset( $enabled ) && $enabled == "yes" ) {
				...
				<div class="p-woo-pagseguro-price">
					...
					printf(__( 'Ou em até <span>%sx</span> de <span>%s</span>', 'wc-pagseguro-parceled' ), $wc_product_parceled_installment, $this->wc_product_parceled() ); ?><br/>
					<span><strong><?php _e( 'Sem Juros - PagSeguro', 'wc-pagseguro-parceled' ); ?></strong></span>
					</div>
				</div>
				<?php
					}
				}
				if ( isset( $installment_table_single ) && $installment_table_single == "yes" && wc_get_price_including_tax( $product, $args ) > $minimum_installment ) { ?>
					<div class="box-parceled">
						<h2><?php _e('Parcelamento PagSeguro', 'wc-pagseguro-parceled' ); ?></h2>
						<div class="left">
			...
			if( isset( $enabled ) && $enabled == "yes" ) {			
				...
				<tr>
					<th colspan="2" class="p-woo-pagseguro-price">
						...
						<div class="p-woo-pagseguro-installment">
							<?php printf( __( '* Pague suas compras em até<br/> %sx sem juros com PagSeguro em nossa loja.', 'wc-pagseguro-parceled'), $installment ); ?>
						</div>
					</th>
				</tr>
				<?php
